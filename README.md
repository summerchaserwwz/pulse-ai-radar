...
[![Cycle](https://img.shields.io/badge/CAPTURE-5_MIN-34d399?style=for-the-badge&labelColor=101412)](https://github.com/summerchaserwwz/pulse-ai-radar)
[![History](https://img.shields.io/badge/HISTORY-CURSOR_PAGED-34d399?style=for-the-badge&labelColor=101412)](https://github.com/summerchaserwwz/pulse-ai-radar)
...
```text
┌─ PULSE/AI ───────────────────────────────────────────────────────────────┐
│  GLOBAL SIGNALS → 中文翻译 → 证据聚类 → 质量门 → 个性化排序               │
│  Cloudflare Cron  5m     D1 history ∞     Email required  NO           │
└─────────────────────────────────────────────────────────────────────────┘
```
...
<table>
  <tr>
    <td width="50%"><img src="./docs/assets/pulse-ai-today.png" alt="PULSE/AI 今日信号界面" /></td>
    <td width="50%"><img src="./docs/assets/pulse-ai-radar.png" alt="PULSE/AI 全球雷达界面" /></td>
  </tr>
  <tr>
    <td align="center"><strong>今日信号 · Today</strong></td>
    <td align="center"><strong>完整历史 · Full radar</strong></td>
  </tr>
</table>

### 30 秒看懂

| 你遇到的问题 | PULSE/AI 的处理方式 |
| --- | --- |
| 全球 AI 信息太散 | 每 5 分钟自动读取已登记的 RSS / Atom，不依赖 Codex 或浏览器在线 |
| 英文信息来不及看 | 自动生成中文标题、摘要和“为什么重要”，同时保留原文 |
| 同一新闻刷屏十遍 | 指纹 + Vectorize 把多篇证据聚成一个事件 |
| 不知道消息靠不靠谱 | 官方性、权威度、多源证据、置信度与隔离机制共同把关 |
| 历史事件越积越多 | D1 长期积累，Web API 使用游标分页，可持续加载而不是截断前 100 条 |
| 不想再注册邮件 | 网站与私有 RSS 都可直接使用，默认产品流不需要邮箱 |
...
在 `SOURCES` 中添加一个定义，部署 Radar Worker 后，它会自动进入 5 分钟采集周期：
...
```ts
{
  ...
  includeTerms: ["model", "agent", "safety"],
}
```

`SOURCE_HOST_ALLOWLIST` 会从注册表自动生成。X、Reddit、YouTube、GitHub Trending 等非 Feed 平台需要单独的 API 连接器，不能仅靠填写 URL 接入。
...
<p align="center">
  <img src="./docs/assets/pulse-ai-pipeline.png" alt="PULSE/AI 自动采集数据管线" width="92%" />
</p>
...
网站 API 默认每页返回 24 条，单页保持轻量，但总历史没有 100 条上限：

```bash
curl 'https://pulse-ai-web.sumerchaser.workers.dev/api/radar?limit=24'

# 将响应中的 page.nextCursor 传入下一页
curl 'https://pulse-ai-web.sumerchaser.workers.dev/api/radar?limit=24&cursor=...'
```

响应会返回 `page.total`、`page.hasMore` 和 `page.nextCursor`。Radar Worker 的 `/v1/radar` 同样支持通过 `offset` 持续翻页。
...
| Problem | What PULSE/AI does |
| --- | --- |
| Global AI news is fragmented | Cloudflare Cron reads registered RSS / Atom feeds every five minutes |
| English updates take time | Produces Chinese titles, summaries, and impact notes while preserving originals |
| The same launch appears everywhere | Fingerprints and Vectorize merge related evidence into one event |
| Trust is unclear | Source authority, independent evidence, confidence, and quarantine gates filter output |
| History keeps growing | D1 persists events; cursor pagination exposes the full history beyond the first 100 |
| Email is unnecessary | The website and private RSS work without an email provider |
...
| Full history | Cursor-paged D1 history with no 100-event ceiling |
...
Register an HTTPS RSS / Atom feed in [`services/radar/src/sources.ts`](./services/radar/src/sources.ts), set its authority, language, official status, and optional keyword filter, then redeploy the Radar Worker. Its hostname is automatically added to the source allowlist. Social platforms and non-feed services require dedicated API connectors.
...
`GET /api/radar?limit=24` returns `page.total`, `page.hasMore`, and `page.nextCursor`. Pass the cursor into the next request to traverse the entire eligible history. The 50-item per-request guard protects latency; it is not a total-history cap.
